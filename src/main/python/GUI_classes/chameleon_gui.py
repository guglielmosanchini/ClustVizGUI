...
from clustviz._chameleon.graphtools import knn_graph, get_cluster, connecting_edges
from clustviz._chameleon.chameleon import rebuild_labels, merge_score
...
class CHAMELEON_class(StartingGui):
    ...
    def cluster_gui(self, df, k, knn=10, m=30, alpha=2.0, save_plots=None):
        ...
        graph = self.pre_part_graph_gui(
            graph=graph, canvas=self.canvas_up, ax=self.ax1, k=m, df=df, plotting=True
        )
